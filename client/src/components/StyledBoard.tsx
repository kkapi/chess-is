...
export const StyledBoard = () => {
  ...
  useEffect(() => {
	...
	return (
		<div className="container w-[100wv] min-h-[80vh] flex justify-center items-center flex-col">
			<div className="flex flex-col md:flex-row justify-center items-center md:gap-12">
				<div className="border rounded-md  w-[400px] md:w-[500px] min-h-[200px] p-8 flex flex-col justify-center items-center gap-5">
					<div className="font-bold  text-xl md:text-4xl">
						{resultMessage ? <>{resultMessage}</> : <span>Анализ</span>}
					</div>

					<div className='w-4/5 h-[180px]'>
						<div>
							Оценка позиции:{' '}
						...
						<h5>{bestLine.length ? <>Лучшая линия: {bestLine.slice(0, 40)} . . .</> : ''}</h5>
            <div>{err && <>Некорректный формат</>}</div>
						<input
